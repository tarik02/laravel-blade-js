...
import { Node, NodeContainer, NodeFunction, NodeWithoutPosition } from './Node';
...
export const parse = (lexer: Lexer, parserConfig?: Partial<ParserConfig>): Node => {
  ...
  const error = (message: string, position?: ScopedPosition): never => {
  ...
  const parser = <N extends NodeWithoutPosition | undefined, T = []>(
    fn: (...args: T[]) => N,
  ): ((...args: T[]) => (N & ScopedPosition) | undefined) => {
    return (...args: T[]): (N & ScopedPosition) | undefined => {
  ...
  const parseAny = parser((): NodeWithoutPosition | undefined => {
